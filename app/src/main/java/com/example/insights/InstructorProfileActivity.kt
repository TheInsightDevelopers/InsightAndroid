...
class InstructorProfileActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Button>(R.id.instructor_profile_signout).setOnClickListener {
            ...
            finish()
            Toast.makeText(this@InstructorProfileActivity,"You have been Logged Out", Toast.LENGTH_SHORT).show()
        }
                ...
                if(task.isSuccessful){
                    FirebaseAuth.getInstance().currentUser.delete().addOnCompleteListener { task2 ->
                        if(task2.isSuccessful){
                            progressBar.hide()
                            Toast.makeText(this@InstructorProfileActivity,"Your account and data has been deleted",Toast.LENGTH_SHORT).show()
                            startActivity(Intent(this@InstructorProfileActivity,LoginPage::class.java))
                            finish()
                        }
                        else{
                            progressBar.hide()
                            Toast.makeText(this@InstructorProfileActivity,"An error Occurred",Toast.LENGTH_SHORT).show()
                        }
                    }
                }
                else{
                    Toast.makeText(this@InstructorProfileActivity,"An error Occured",Toast.LENGTH_SHORT).show()
                    progressBar.hide()
        ...
        findViewById<Button>(R.id.instructor_profile_editprofile).setOnClickListener {
            startActivity(Intent(this@InstructorProfileActivity,InstructorEditProfileActivity::class.java))
        }
