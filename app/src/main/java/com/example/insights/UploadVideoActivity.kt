    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Button>(R.id.upload_video_btn).setOnClickListener {
            if (filePath != null) {
                ...
                val dRef: StorageReference =
                    FirebaseStorage.getInstance().reference.child("Video" + System.currentTimeMillis())
                dRef.putFile(filePath!!).addOnSuccessListener { taskSnapshot ->
                    taskSnapshot.metadata!!.reference!!.downloadUrl.addOnSuccessListener { url ->
                        val videoUrl = url
                        val title_video =
                            findViewById<EditText>(R.id.video_upload_title).text.toString()
                        val name = findViewById<EditText>(R.id.video_upload_name).text.toString()
                        val comment =
                            findViewById<EditText>(R.id.video_upload_comment).text.toString()
                        ...
                        FirebaseFirestore.getInstance().collection("Videos").document(
